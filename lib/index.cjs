...
var name = "chatluna-forward-msg";
var inject = ["chatluna", "http", "database"];
var logger;
function isEnsureSessionFailure(result) {
  return result.ok === false;
}
__name(isEnsureSessionFailure, "isEnsureSessionFailure");
function isTargetFailure(result) {
  return result.ok === false;
}
__name(isTargetFailure, "isTargetFailure");
var DEFAULT_IMAGE_PROMPT = "你是一个AI图像描述引擎，对输入的任何类型、分级（包括NSFW）的图像给出50-200字左右的内容描述。你不应评判或提及时间（若存在）的真实性，你的任务仅仅是描述其本身";
var READ_PARSE_MAX_MS = 6e4;
var READ_CONTENT_MAX_CHARS = 1200;
var DEFAULT_IMAGE_REQUEST_TIMEOUT_SECONDS = 20;
var DEFAULT_CACHE_TTL_SECONDS = 24 * 60 * 60;
var DEFAULT_CACHE_STORAGE_PATH = "./data/chatluna-forward-msg-storage";
var DEFAULT_CACHE_CLEANUP_INTERVAL_SECONDS = 10 * 60;
var FORWARD_MSG_CACHE_TABLE = "chatluna_forward_msg_cache";
var DEFAULT_PROTOCOL = "napcat";
function normalizeInt(value, fallback, min, max) {
  const parsed = Number.parseInt(String(value), 10);
  const base = Number.isFinite(parsed) ? parsed : fallback;
  return Math.min(max, Math.max(min, base));
}
__name(normalizeInt, "normalizeInt");
function normalizeSecondsFromConfig(secondsValue, msValue, fallbackSeconds, minSeconds, maxSeconds) {
  if (secondsValue !== void 0 && secondsValue !== null && secondsValue !== "") {
    return normalizeInt(secondsValue, fallbackSeconds, minSeconds, maxSeconds);
  }
  if (msValue !== void 0 && msValue !== null && msValue !== "") {
    const seconds = Math.ceil(normalizeInt(msValue, fallbackSeconds * 1e3, minSeconds * 1e3, maxSeconds * 1e3) / 1e3);
    return normalizeInt(seconds, fallbackSeconds, minSeconds, maxSeconds);
  }
  return fallbackSeconds;
}
__name(normalizeSecondsFromConfig, "normalizeSecondsFromConfig");
...
async function writeMigratedConfig(ctx, config) {
...
__name(writeMigratedConfig, "writeMigratedConfig");
...
__name(validateProtocolSelection, "validateProtocolSelection");
function getReadToolConfig(config) {
  ...
  return {
    ...
    description: trimText(tool.description) || "读取 OneBot 合并转发消息内容。支持递归解析嵌套合并转发，提取文本、图片、视频与文件，并可调用多模态模型生成图片描述。",
...
__name(getReadToolConfig, "getReadToolConfig");
function getSendToolConfig(config) {
  const tool = config?.sendTool || {};
  return {
    enable: tool.enable !== false,
    name: trimText(tool.name) || "send_forward_msg",
    description: trimText(tool.description) || "以 Bot 自身身份发送合并转发消息，支持文本和图片 URL。未指定目标时默认发送到当前会话（群聊自动使用当前群号，私聊自动使用当前用户）。",
    botDisplayName: trimText(tool.botDisplayName ?? config?.botDisplayName)
  };
}
__name(getSendToolConfig, "getSendToolConfig");
function getFakeToolConfig(config) {
  const tool = config?.fakeTool || {};
  return {
    enable: tool.enable !== false,
    name: trimText(tool.name) || "send_fake_msg",
    description: trimText(tool.description) || "以任意 senderId/senderName 伪造消息节点发送合并转发，支持文本和图片 URL。未指定目标时默认发送到当前会话（群聊自动使用当前群号，私聊自动使用当前用户）。"
  };
}
__name(getFakeToolConfig, "getFakeToolConfig");
function getDescribeImageToolConfig(config) {
  ...
  return {
    ...
    description: trimText(tool.description) || "对指定 URL 图片生成图像描述。可传入 requirement（描述要求），系统会把该要求追加到图片描述提示词末尾。"
  };
}
__name(getDescribeImageToolConfig, "getDescribeImageToolConfig");
function getImageServiceConfig(config) {
  ...
  return {
    ...
    requestTimeoutMs: requestTimeoutSeconds * 1e3
  };
}
__name(getImageServiceConfig, "getImageServiceConfig");
function getCacheServiceConfig(config) {
  ...
  return {
    ...
    ttlSeconds,
    ttlMs: ttlSeconds * 1e3,
    storagePath: trimText(cache.storagePath) || DEFAULT_CACHE_STORAGE_PATH,
    cleanupIntervalSeconds,
    cleanupIntervalMs: cleanupIntervalSeconds * 1e3
  };
}
__name(getCacheServiceConfig, "getCacheServiceConfig");
var ForwardMsgCacheService = class {
  static {
    __name(this, "ForwardMsgCacheService");
  }
  ctx;
  config;
  initialized;
...
__name(buildImageDescriptionCacheKey, "buildImageDescriptionCacheKey");
function apply(ctx, config) {
  logger = (0, import_logger.createLogger)(ctx, name);
  ...
  const plugin = new import_chat.ChatLunaPlugin(ctx, config, "forward-msg", false);
  ...
  ctx.on("ready", async () => {
    ...
    const storagePath = import_path.default.resolve(process.cwd(), cacheConfig.storagePath);
    try {
      const stat = await import_promises.default.stat(storagePath);
      if (stat.isDirectory()) {
        await import_promises.default.rm(storagePath, { recursive: true, force: true });
...
function resolveModelName(ctx, config) {
  ...
  const allModels = ctx.chatluna.platform.listAllModels(import_types.ModelType.llm).value || [];
  const imageModel = allModels.find(
    (model) => Array.isArray(model.capabilities) && model.capabilities.includes(import_types.ModelCapabilities.ImageInput)
  );
...
__name(getSessionFromRunnable, "getSessionFromRunnable");
...
__name(ensureOnebotSession, "ensureOnebotSession");
...
__name(parseMimeFromDataUrl, "parseMimeFromDataUrl");
function isBase64DataUrl(value) {
  return /^data:[^;,]+(?:;[^,]*)?;base64,/i.test(trimText(value));
}
__name(isBase64DataUrl, "isBase64DataUrl");
...
__name(estimateBase64DataUrlBytes, "estimateBase64DataUrlBytes");
...
__name(renderBase64UrlTag, "renderBase64UrlTag");
...
__name(sanitizeBase64Urls, "sanitizeBase64Urls");
function getErrorMessage(error) {
  return sanitizeBase64Urls(error?.message || String(error));
}
__name(getErrorMessage, "getErrorMessage");
function formatErrorForLog(error) {
  const name2 = trimText(error?.name) || "Error";
  ...
  return [name2, code ? `code=${code}` : "", status ? `status=${status}` : "", message].filter(Boolean).join(", ");
}
__name(formatErrorForLog, "formatErrorForLog");
...
__name(classifyImageError, "classifyImageError");
...
__name(maskLongUrl, "maskLongUrl");
...
__name(isHttpOrHttpsUrl, "isHttpOrHttpsUrl");
function extractImageIdentityHash(data) {
  const candidates = [
    data?.hash,
    data?.sha256,
    data?.sha1,
...
__name(extractImageIdentityHash, "extractImageIdentityHash");
async function readImageAsDataUrlWithMeta(ctx, url, timeout) {
  ...
  const mime = (0, import_string.getImageType)(buffer) || "image/jpeg";
  return {
...
__name(readImageAsDataUrlWithMeta, "readImageAsDataUrlWithMeta");
async function readImagesAsDataUrlsByHash(ctx, urls, concurrency, timeout) {
  const refs = (urls || []).map((url, index) => ({ url: trimText(url), index })).filter((item) => item.url);
  ...
  const dataUrlByHash = /* @__PURE__ */ new Map();
...
__name(readImagesAsDataUrlsByHash, "readImagesAsDataUrlsByHash");
async function describeImageWithModel({
  ...
  if (!Array.isArray(model.modelInfo?.capabilities) || !model.modelInfo.capabilities.includes(import_types.ModelCapabilities.ImageInput)) {
  ...
  let result;
  try {
    result = await model.invoke([new import_messages.HumanMessage({ content })]);
  } catch (error) {
    const reason = classifyImageError(error);
    logger.warn(
  ...
  return (0, import_string.getMessageContent)(result.content);
}
__name(describeImageWithModel, "describeImageWithModel");
...
__name(toTextSegment, "toTextSegment");
...
__name(toImageSegment, "toImageSegment");
...
__name(parseToolMessages, "parseToolMessages");
...
__name(normalizeImageUrls, "normalizeImageUrls");
...
__name(escapeMarkdownAlt, "escapeMarkdownAlt");
...
__name(toMarkdownImage, "toMarkdownImage");
...
__name(toMarkdownMediaLink, "toMarkdownMediaLink");
...
function pickTarget(session, input) {
  ...
  if (groupId || targetType === "group") {
    ...
    if (!id) return { ok: false, error: "Missing groupId. Provide groupId or run inside a group session." };
    return { ok: true, type: "group", id };
  }
  if (userId || targetType === "private") {
    const id = userId || trimText(session.userId);
    if (!id) return { ok: false, error: "Missing userId for private target." };
...
async function fetchForwardMessages(internal, messageId) {
  ...
  const requestForwardByPayload = /* @__PURE__ */ __name(async (payload) => {
    try {
  ...
  const getMessageById = /* @__PURE__ */ __name(async (id) => {
    try {
  ...
  const collectForwardIdsFromSegments = /* @__PURE__ */ __name((segments) => {
    const result = [];
...
__name(extractReplyIdFromRawMessage, "extractReplyIdFromRawMessage");
async function fetchBotQQNickname(internal) {
  try {
    if (typeof internal.getLoginInfo === "function") {
      const data2 = await internal.getLoginInfo();
      return trimText(data2?.nickname);
    }
...
__name(fetchBotQQNickname, "fetchBotQQNickname");
...
__name(fetchGroupMember, "fetchGroupMember");
function collectNicknameCandidates(member, userId, fallbackNames = []) {
  ...
  return candidates.map((name2) => trimText(name2)).filter(Boolean);
}
__name(collectNicknameCandidates, "collectNicknameCandidates");
async function resolveUserDisplayName({
  ...
  const name2 = collectNicknameCandidates(member, normalizedUserId, fallbackNames)[0] || normalizedUserId;
  if (nameCache) {
    nameCache.set(normalizedUserId, name2);
  }
  return name2;
}
__name(resolveUserDisplayName, "resolveUserDisplayName");
async function resolveBotDisplayName({ internal, session, target, config, botId, nameCache }) {
  ...
  return fallbackNames[0] || botId;
}
__name(resolveBotDisplayName, "resolveBotDisplayName");
function extractSegmentsFromContent(content) {
  if (Array.isArray(content)) return content;
  if (typeof content === "string") return [toTextSegment(content)];
  return [];
}
__name(extractSegmentsFromContent, "extractSegmentsFromContent");
...
__name(dedupeOrdered, "dedupeOrdered");
function formatMessageTime(value) {
  ...
  if (typeof value === "number") {
    ...
    if (Number.isFinite(numeric)) {
      ms = numeric < 1e12 ? numeric * 1e3 : numeric;
    } else {
...
__name(formatMessageTime, "formatMessageTime");
function extractMessageId(message) {
  return trimText(message?.message_id) || trimText(message?.id) || trimText(message?.real_id) || trimText(message?.message_seq) || "";
}
...
__name(collectMessageIndexKeys, "collectMessageIndexKeys");
...
__name(normalizeReadBudget, "normalizeReadBudget");
...
__name(shouldStopReadParse, "shouldStopReadParse");
...
__name(getReadBudgetRemainingMs, "getReadBudgetRemainingMs");
...
__name(getMessageSegments, "getMessageSegments");
function isStickerImage(type, data) {
  return type === "mface" || data.sticker === true || data.subType === 1 || data["sub-type"] === 1 || data.sub_type === 1 || data.summary === "[动画表情]";
}
__name(isStickerImage, "isStickerImage");
...
__name(collectImageTasks, "collectImageTasks");
function appendImageResult(image) {
  for (const ref of image.refs || []) {
    ref.entry.contentParts.push(toMarkdownImage(image.description, ref.source));
  }
}
__name(appendImageResult, "appendImageResult");
async function describeImageTasks(args) {
...
__name(describeImageTasks, "describeImageTasks");
async function parseForwardMessages({
  ...
  for (const message of messages || []) {
    ...
    for (const segment of segments) {
      ...
      if (type === "reply") {
        const quoteId = trimText(data.id) || trimText(data.message_id) || trimText(data.real_id) || trimText(data.message_seq) || trimText(data.real_seq) || trimText(data.seq);
    ...
    if (imageSources.length) {
      const canDescribeImages = readToolConfig.describeImageInRead && !shouldStopReadParse(parseBudget) && getReadBudgetRemainingMs(parseBudget) > 3e3;
  ...
  for (const item of result) {
    const forwardIds = Array.isArray(item.forward) ? item.forward.map((child) => trimText(child?.messageId)).join(",") : "";
...
__name(parseForwardMessages, "parseForwardMessages");
var ReadForwardMsgTool = class extends import_tools.StructuredTool {
  static {
    __name(this, "ReadForwardMsgTool");
  }
  deps;
  name;
  description;
  ...
  constructor(deps) {
    ...
    this.schema = import_zod.z.object({
      messageId: import_zod.z.string().min(1).describe("目标合并转发消息 ID。"),
      maxDepth: import_zod.z.number().int().min(1).max(8).optional().describe("嵌套合并转发解析层数（含读取的记录本身），默认使用插件配置值。")
    });
  }
  async _call(input, _manager, runnable) {
    try {
      const checked = ensureOnebotSession(runnable, this.deps.config);
      if (isEnsureSessionFailure(checked)) return checked.error;
      ...
      const maxDepth = Number.isInteger(input.maxDepth) ? input.maxDepth : readToolConfig.maxParseDepth;
      ...
      if (Array.isArray(cached)) {
        logger.info(
          "read_forward_msg cache hit, count: %d, messageId: %s, maxDepth: %d",
          cached.length,
          messageId,
          maxDepth
        );
      ...
      const parsed = await parseForwardMessages({
        ...
        depth: nestedDepth,
        visited: /* @__PURE__ */ new Set([messageId]),
        budget
      });
      if (budget.timedOut) {
      ...
      logger.info(
        "read_forward_msg parsed count: %d, truncated: %s, costMs: %d, messageId: %s",
        parsed.length,
        budget.truncated ? "true" : "false",
        Date.now() - budget.startedAt,
        messageId
      );
      const normalizeOutputEntry = /* @__PURE__ */ __name((item) => {
        const output = {
      ...
      logger.info(
        "read_forward_msg cache set, count: %d, messageId: %s, maxDepth: %d",
        messages.length,
        messageId,
        maxDepth
      );
...
var SendForwardMsgTool = class extends import_tools.StructuredTool {
  static {
    __name(this, "SendForwardMsgTool");
  }
  deps;
  name;
  description;
  ...
  constructor(deps) {
    ...
    this.schema = import_zod.z.object({
      ...
      targetType: import_zod.z.enum(["group", "private"]).optional().describe("发送目标类型，不填则自动判断（群聊默认 group，私聊默认 private）。"),
      groupId: import_zod.z.string().optional().describe("目标群号，不填则使用当前群号。"),
      userId: import_zod.z.string().optional().describe("目标用户 QQ 号，用于私聊发送；不填则使用当前会话用户。在群聊中设置 targetType 为 private 时必填。")
    });
  }
  async _call(input, _manager, runnable) {
    try {
      ...
      if (isTargetFailure(targetInfo)) return targetInfo.error;
...
var SendFakeMsgTool = class extends import_tools.StructuredTool {
  static {
    __name(this, "SendFakeMsgTool");
  }
  deps;
  name;
  description;
  ...
  constructor(deps) {
    ...
    this.schema = import_zod.z.object({
      messages: import_zod.z.union([
        ...
        import_zod.z.array(
          import_zod.z.object({
            senderId: import_zod.z.string().optional(),
            senderName: import_zod.z.string().optional(),
            text: import_zod.z.string().optional(),
            images: import_zod.z.array(import_zod.z.string()).optional()
          })
        )
...
var DescribeImageByUrlTool = class extends import_tools.StructuredTool {
  static {
    __name(this, "DescribeImageByUrlTool");
  }
  deps;
  name;
  description;
  ...
  constructor(deps) {
    ...
    this.description = cfg.description;
    this.schema = import_zod.z.object({
      url: import_zod.z.string().optional().describe("图片 URL。"),
      requirement: import_zod.z.string().optional().describe("补充描述要求，会追加到提示词末尾。")
    });
  }
  async _call(input) {
    try {
      ...
      const imageConfig = getImageServiceConfig(this.deps.config);
      const finalPrompt = requirement ? `${imageConfig.prompt}

补充描述要求：${requirement}` : imageConfig.prompt;
...
var Config = import_koishi.Schema.intersect([
  import_koishi.Schema.object({
    protocolService: import_koishi.Schema.object({
      protocol: import_koishi.Schema.union([
    ...
    readTool: import_koishi.Schema.object({
      enable: import_koishi.Schema.boolean().default(true).description("是否启用（读取并解析合并转发消息，只提取文本和图片；图片可调用多模态模型生成描述）"),
      name: import_koishi.Schema.string().default("read_forward_msg").description("读取合并转发工具名（提供给 ChatLuna 调用）"),
      description: import_koishi.Schema.string().default("读取 OneBot 合并转发消息内容。支持递归解析嵌套合并转发，提取文本、图片、视频与文件，并可调用多模态模型生成图片描述。").description("读取合并转发工具描述"),
      maxParseDepth: import_koishi.Schema.number().min(1).max(8).default(3).description("嵌套合并转发解析层数（含读取的记录本身）"),
      describeImageInRead: import_koishi.Schema.boolean().default(true).description("读取合并转发时是否为图片生成描述")
    }).description("读取工具"),
    sendTool: import_koishi.Schema.object({
      enable: import_koishi.Schema.boolean().default(true).description("是否启用（以 Bot 自身身份发送合并转发，支持文本与图片 URL，自动转 Base64 上传）"),
      name: import_koishi.Schema.string().default("send_forward_msg").description("Bot 身份发送合并转发工具名（提供给 ChatLuna 调用）"),
      description: import_koishi.Schema.string().default("以 Bot 自身身份发送合并转发消息，支持文本和图片 URL。").description("Bot 身份发送合并转发工具描述"),
      botDisplayName: import_koishi.Schema.string().default("").description("发送时显示昵称，留空则自动推断")
    }).description("发送工具"),
    fakeTool: import_koishi.Schema.object({
      enable: import_koishi.Schema.boolean().default(true).description("是否启用（以任意用户 ID/昵称伪造节点发送合并转发，支持文本与图片 URL，自动转 Base64 上传）"),
      name: import_koishi.Schema.string().default("send_fake_msg").description("伪造身份发送合并转发工具名（提供给 ChatLuna 调用）"),
      description: import_koishi.Schema.string().default("以任意 senderId/senderName 伪造消息节点发送合并转发，支持文本和图片 URL。").description("伪造身份发送合并转发工具描述")
    }).description("伪造工具"),
    describeImageTool: import_koishi.Schema.object({
      enable: import_koishi.Schema.boolean().default(true).description("是否启用（按 URL 获取图像描述，可附加补充描述要求）"),
      name: import_koishi.Schema.string().default("describe_image_by_url").description("按 URL 获取图像描述工具名（提供给 ChatLuna 调用）"),
      description: import_koishi.Schema.string().default("对指定 URL 图片生成图像描述。可传入 requirement（描述要求），系统会把该要求追加到图片描述提示词末尾。").description("按 URL 获取图像描述工具描述")
    }).description("图像描述工具"),
    imageService: import_koishi.Schema.object({
      model: import_koishi.Schema.dynamic("model").default("无").description("用于图片描述的多模态模型"),
      prompt: import_koishi.Schema.string().role("textarea").default(DEFAULT_IMAGE_PROMPT).description("图片描述提示词"),
      skipSticker: import_koishi.Schema.boolean().default(true).description("图片描述是否跳过表情包（sticker/mface），开启后仅保留图片占位，不调用多模态模型"),
      taskConcurrency: import_koishi.Schema.number().min(1).max(100).default(20).description("图片任务并发数（读取描述、发送下载共用）"),
      requestTimeoutSeconds: import_koishi.Schema.number().min(1).max(120).default(DEFAULT_IMAGE_REQUEST_TIMEOUT_SECONDS).description("下载图片请求超时时间（秒）")
    }).description("图片描述服务"),
    cacheService: import_koishi.Schema.object({
      enable: import_koishi.Schema.boolean().default(true).description("是否启用缓存。启用后同 message_id 会优先返回读取结果缓存，图片描述会按内容哈希缓存，减少重复请求"),
      ttlSeconds: import_koishi.Schema.number().min(60).max(30 * 24 * 60 * 60).default(DEFAULT_CACHE_TTL_SECONDS).description("缓存有效期（秒），默认 1 天"),
      storagePath: import_koishi.Schema.path({ filters: ["directory"] }).default(DEFAULT_CACHE_STORAGE_PATH).description("旧版本地缓存目录。插件启动前会检测并删除该目录"),
      cleanupIntervalSeconds: import_koishi.Schema.number().min(60).max(24 * 60 * 60).default(DEFAULT_CACHE_CLEANUP_INTERVAL_SECONDS).description("后台清理过期缓存的最小间隔（秒）")
    }).description("缓存服务")
  })
]);
var usage = `
