...
function getSendToolConfig(config) {
  ...
  return {
    ...
    description: trimText(tool.description) || "以 Bot 自身身份发送合并转发消息，支持文本和图片 URL。",
...
function getFakeToolConfig(config) {
  ...
  return {
    ...
    description: trimText(tool.description) || "以任意 senderId/senderName 伪造消息节点发送合并转发，支持文本和图片 URL。"
  };
}
...
var SendForwardMsgTool = class extends import_tools.StructuredTool {
  ...
  constructor(deps) {
    ...
    this.schema = import_zod.z.object({
      ...
      targetType: import_zod.z.enum(["group", "private"]).optional().describe("发送目标类型。"),
      groupId: import_zod.z.string().optional().describe("目标群号。"),
      userId: import_zod.z.string().optional().describe("目标用户 QQ 号。")
    });
  }
  async _call(input, _manager, runnable) {
    try {
