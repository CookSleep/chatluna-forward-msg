...
function getImageServiceConfig(config) {
  ...
  return {
    ...
    requestTimeoutMs: normalizeInt(image.requestTimeoutMs ?? config?.requestTimeoutMs, 20000, 1000, 120000),
  }
}
  ...
  ctx.on('ready', async () => {
    ...
    if (readTool.enable) {
      plugin.registerTool(readTool.name, {
        ...
        createTool() {
          return new ReadForwardMsgTool({ ctx, config, ensureImageModelRef })
        },
      })
    }
...
async function parseForwardMessages({
  ...
  if (imageTasks.length > 0) {
    ...
    logger.info(
      'image parse global batch start, imageCount=%d, uniqueImageCount=%d, dedupSaved=%d, concurrency=%d, timeoutMs=%d',
      imageTasks.length,
...
class ReadForwardMsgTool extends StructuredTool {
  ...
  async _call(input, _manager, runnable) {
    try {
      ...
      const parsedMessageIndex = new Map()
      const indexParsedTree = (items) => {
        for (const item of items || []) {
          const keys = dedupeOrdered([
            trimText(item?.messageId),
            ...(Array.isArray(item?.indexKeys) ? item.indexKeys.map((x) => trimText(x)) : []),
          ]).filter(Boolean)
          for (const key of keys) {
            if (!parsedMessageIndex.has(key)) {
              parsedMessageIndex.set(key, item)
            }
          }
          if (Array.isArray(item?.forward) && item.forward.length > 0) {
            indexParsedTree(item.forward)
          }
        }
      }
      indexParsedTree(parsed)

      const collectQuoteIds = (items, output = new Set()) => {
        for (const item of items || []) {
          const quoteId = trimText(item?.quote)
          if (quoteId) output.add(quoteId)
          if (Array.isArray(item?.forward) && item.forward.length > 0) {
            collectQuoteIds(item.forward, output)
          }
        }
        return output
      }

      const quoteIds = Array.from(collectQuoteIds(parsed))
      const unresolvedQuoteIds = quoteIds.filter((id) => !parsedMessageIndex.has(id))
      const externalReplyIndex = new Map()
      if (unresolvedQuoteIds.length > 0) {
        const fetched = await mapWithConcurrency(
          unresolvedQuoteIds,
          8,
          async (id) => {
            const data = await fetchMessageById(internal, id)
            return { id, message: normalizeOnebotMessageObject(data) }
          },
        )
        for (const item of fetched) {
          if (item?.id && item?.message) {
            const normalized = normalizeExternalReplyMessage(item.message)
            if (normalized) {
              externalReplyIndex.set(item.id, normalized)
            }
          }
        }
      }

      const toReplyJson = (item) => {
        const output = {
          messageId: trimText(item?.messageId),
          nickname: trimText(item?.nickname),
          id: trimText(item?.userId),
          time: trimText(item?.time),
          content: trimText(item?.content),
        }
        if (Array.isArray(item?.forward) && item.forward.length > 0) {
          output.forward = item.forward.map((child) => toReplyJson(child))
        }
        return output
      }
      ...
      const normalizeOutputEntry = (item) => {
        ...
        const quoteId = trimText(item?.quote)
        if (quoteId) {
          const quoted = parsedMessageIndex.get(quoteId)
          if (quoted) {
            output.replyTo = toReplyJson(quoted)
          } else {
            const external = externalReplyIndex.get(quoteId)
            if (external) {
              output.replyTo = toReplyJson(external)
            } else {
              output.replyTo = { messageId: quoteId }
            }
          }
        }
...
const Config = Schema.intersect([
  Schema.object({
    readTool: Schema.object({
      ...
      maxParseDepth: Schema.number().min(1).max(8).default(3).description('默认嵌套合并转发解析层数。'),
    ...
    imageService: Schema.object({
      ...
      requestTimeoutMs: Schema.number().min(1000).max(120000).default(20000).description('下载图片请求超时时间（毫秒）。'),
    }).description('图片描述服务'),
  }),
])
