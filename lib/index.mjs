...
function resolveModelName(ctx, config) {
  ...
  const imageModel = allModels.find(
    (model) => Array.isArray(model.capabilities) && model.capabilities.includes(ModelCapabilities.ImageInput)
  );
...
async function describeImageWithModel({
  ...
  if (!Array.isArray(model.modelInfo?.capabilities) || !model.modelInfo.capabilities.includes(ModelCapabilities.ImageInput)) {
...
var ReadForwardMsgTool = class extends StructuredTool {
  static {
  ...
  constructor(deps) {
    ...
    this.schema = z.object({
      ...
      maxDepth: z.number().int().min(1).max(8).optional().describe("嵌套合并转发解析层数（含读取的记录本身），默认使用插件配置值。")
    });
  }
...
var SendForwardMsgTool = class extends StructuredTool {
  static {
...
var SendFakeMsgTool = class extends StructuredTool {
  static {
...
var DescribeImageByUrlTool = class extends StructuredTool {
  static {
