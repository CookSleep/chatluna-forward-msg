...
type PluginContext = {
  ...
  chatluna: {
    ...
    platform: {
      listAllModels: (type: ModelTypeValue) => { value?: ChatModelInfoLite[] }
    }
  }
}
...
type NapcatInternal = {
  _get?: <T = unknown>(action: string, params?: Record<string, unknown>) => Promise<T>
  ...
  sendAction?: <T = unknown>(action: string, params?: Record<string, unknown>) => Promise<T>
  getMsg?: (payload: string | Record<string, unknown>) => Promise<unknown>
  getLoginInfo?: () => Promise<{ nickname?: unknown }>
  getGroupMemberInfo?: (groupId: string, userId: string, noCache?: boolean) => Promise<NapcatGroupMember | null>
}
...
type EnsureSessionFailure = { ok: false, error: string }
type EnsureSessionSuccess = { ok: true, session: NapcatSession, internal: NapcatInternal }
type EnsureSessionResult = EnsureSessionFailure | EnsureSessionSuccess
...
type ReadToolDeps = BaseToolDeps & {
...
type ForwardMessage = {
  sender?: { user_id?: unknown, uin?: unknown, nickname?: unknown, card?: unknown }
  user_id?: unknown
  sender_id?: unknown
  nickname?: unknown
  time?: unknown
  ...
  message_seq?: unknown
  real_seq?: unknown
  data?: { content?: any, message?: any, raw_message?: unknown, message_id?: unknown }
}
...
type ParsedForwardEntry = {
  ...
  quote: string
  content: string
  forward: ParsedForwardEntry[]
}

type StagedForwardEntry = {
  messageId: string
  indexKeys: string[]
  nickname: string
  userId: string
  ...
  quote: string
  contentParts: string[]
  forwards: ParsedForwardEntry[]
}

type ReadParseBudget = {
  startedAt: number
  maxMs: number
  truncated: boolean
  timedOut: boolean
...
type TargetFailure = { ok: false, error: string }
type TargetSuccess = { ok: true, type: 'group' | 'private', id: string }
type TargetInfo = TargetFailure | TargetSuccess

function isEnsureSessionFailure(result: EnsureSessionResult): result is EnsureSessionFailure {
  return result.ok === false
}

function isTargetFailure(result: TargetInfo): result is TargetFailure {
  return result.ok === false
}
...
type SendToolInput = {
  messages: string | SendMessageItem[]
  targetType?: 'group' | 'private'
  groupId?: string
  userId?: string
}

type FakeMessageItem = SendMessageItem & {
  senderId?: string
  senderName?: string
}

type FakeToolInput = {
  messages: string | FakeMessageItem[]
  targetType?: 'group' | 'private'
  groupId?: string
  userId?: string
}
...
type ResolveUserDisplayNameArgs = {
  internal: NapcatInternal
...
type ResolveBotDisplayNameArgs = {
  internal: NapcatInternal
...
class ForwardMsgCacheService {
  ...
  async get(key: string): Promise<any | null> {
    ...
    const row: CacheRow | undefined = rows[0]
    if (!row) return null
...
function resolveModelName(ctx: PluginContext, config: PluginConfig): string {
...
function getSessionFromRunnable(runnable: any): NapcatSession | undefined {
  return runnable?.configurable?.session
}

function ensureOnebotSession(runnable: any, config: PluginConfig): EnsureSessionResult {
  const protocolCheck = validateProtocolSelection(config)
  if ('error' in protocolCheck) {
...
async function callApi<T = unknown>(
  ...
  if (typeof internal._get === 'function') {
    return await internal._get<T>(action, params)
  }

  if (typeof internal.request === 'function') {
    return await internal.request<T>(action, params)
  }

  if (typeof internal.callAction === 'function') {
    return await internal.callAction<T>(action, params)
  }

  if (typeof internal.sendAction === 'function') {
    return await internal.sendAction<T>(action, params)
  }
...
async function describeImageWithModel({
  ...
  modelRef: ImageModelRef | undefined
  imagePrompt: string
  dataUrl: string
}): Promise<string> {
  if (!modelRef?.value) {
...
function toTextSegment(text: string): OutgoingSegment {
  return {
    type: 'text',
    data: {
      text: String(text || ''),
...
function toImageSegment(dataUrl: string): OutgoingSegment {
  return {
    type: 'image',
    data: {
      file: dataUrl,
...
function parseToolMessages<T extends object>(messages: unknown): T[] {
...
function normalizeImageUrls(item: SendMessageItem | FakeMessageItem): string[] {
  const urls: string[] = []
...
function pickTarget(session: NapcatSession, input: SendToolInput | FakeToolInput): TargetInfo {
...
async function fetchForwardMessages(internal: NapcatInternal, messageId: string): Promise<ForwardMessage[]> {
  ...
  const extractMessagesArray = (data) => {
    ...
    if (!data || typeof data !== 'object') return null
      ...
      const envelope = data as ForwardMessagesEnvelope
      const candidates = [
      envelope?.messages,
  ...
  const requestForwardByPayload = async (payload: Record<string, unknown>): Promise<ForwardMessage[] | null> => {
    try {
      const data = await callApi<ForwardMessage[] | ForwardMessagesEnvelope>(internal, 'get_forward_msg', payload)
  ...
  const getMessageById = async (id: string): Promise<ForwardMessagesEnvelope | ForwardMessage | null> => {
    try {
      if (typeof internal.getMsg === 'function') {
        return await internal.getMsg(id)
      }
      return await callApi<ForwardMessagesEnvelope | ForwardMessage>(internal, 'get_msg', { message_id: id })
    } catch {
      return null
    }
  }
...
async function fetchMessageById(internal: NapcatInternal, messageId: string): Promise<ForwardMessage | null> {
  const normalized = trimText(messageId)
  ...
  const parsePayload = (payload: string): string | Record<string, unknown> | null => {
  ...
  const pickUsableMessage = (data: unknown): ForwardMessage | null => {
  ...
  for (const payload of payloads) {
    ...
    if (typeof parsed === 'object') {
      try {
        const data = await callApi<ForwardMessage | ForwardMessagesEnvelope>(internal, 'get_msg', parsed)
        const message = pickUsableMessage(data)
        if (message) return message
      } catch { }
    }
...
function normalizeOnebotMessageObject(message: unknown): ForwardMessage | null {
  if (!message || typeof message !== 'object') return null
  const direct = message as ForwardMessage & ForwardMessagesEnvelope
  const nested = direct?.data && typeof direct.data === 'object' ? direct.data : null
  const target: unknown = nested?.message || nested?.raw_message || nested?.message_id ? nested : direct
  if (!target || typeof target !== 'object') return null
  return target as ForwardMessage
}
...
function normalizeExternalReplyMessage(message: unknown) {
...
async function fetchBotQQNickname(internal: NapcatInternal): Promise<string> {
  try {
    if (typeof internal.getLoginInfo === 'function') {
      const data = await internal.getLoginInfo()
      return trimText(data?.nickname)
    }
    const data = await callApi<{ nickname?: unknown }>(internal, 'get_login_info', {})
    return trimText(data?.nickname)
  } catch {
    return ''
  }
...
async function fetchGroupMember(internal: NapcatInternal, groupId: string, userId: string): Promise<NapcatGroupMember | null> {
  try {
    if (!groupId || !userId) return null
    if (typeof internal.getGroupMemberInfo === 'function') {
      return await internal.getGroupMemberInfo(groupId, userId, false)
    }
    return await callApi<NapcatGroupMember>(internal, 'get_group_member_info', {
      group_id: groupId,
      user_id: userId,
      no_cache: false,
    })
...
function collectNicknameCandidates(member: NapcatGroupMember | null, userId: string, fallbackNames: string[] = []): string[] {
  const candidates = [
    member?.card,
    member?.remark,
    member?.displayName,
...
async function resolveUserDisplayName({
  ...
  fallbackNames = [],
  nameCache,
}: ResolveUserDisplayNameArgs): Promise<string> {
...
async function resolveBotDisplayName({ internal, session, target, config, botId, nameCache }: ResolveBotDisplayNameArgs): Promise<string> {
...
function extractSegmentsFromContent(content: unknown): MessageSegment[] {
  if (Array.isArray(content)) return content as MessageSegment[]
  if (typeof content === 'string') return [toTextSegment(content)]
  return []
}

function dedupeOrdered(list: Array<string | number | boolean | null | undefined>): string[] {
...
function extractMessageId(message: ForwardMessage) {
  return (
...
function collectMessageIndexKeys(message: ForwardMessage) {
...
function normalizeReadBudget(budget?: ReadParseBudget): ReadParseBudget {
...
function shouldStopReadParse(budget?: ReadParseBudget): boolean {
...
function getReadBudgetRemainingMs(budget?: ReadParseBudget): number {
  if (!budget) return Number.POSITIVE_INFINITY
  return Math.max(0, budget.maxMs - (Date.now() - budget.startedAt))
}

function getMessageSegments(message: ForwardMessage): MessageSegment[] {
  const fromContent = extractSegmentsFromContent(message?.content) as MessageSegment[]
  if (fromContent.length > 0) return fromContent

  const fromDataContent = extractSegmentsFromContent(message?.data?.content) as MessageSegment[]
  if (fromDataContent.length > 0) return fromDataContent

  const fromMessage = extractSegmentsFromContent(message?.message) as MessageSegment[]
  if (fromMessage.length > 0) return fromMessage

  const fromDataMessage = extractSegmentsFromContent(message?.data?.message) as MessageSegment[]
...
async function parseForwardMessages({
  ...
  visited,
  budget,
}: ParseForwardMessagesArgs): Promise<ParsedForwardEntry[]> {
  const result: ParsedForwardEntry[] = []
  const stagedEntries: StagedForwardEntry[] = []
  const imageTasks: ImageTaskRef[] = []
  ...
  for (const message of messages || []) {
    ...
    const entry: StagedForwardEntry = {
      messageId,
  ...
  const seen = new Set()
  const compact: ParsedForwardEntry[] = []
...
class ReadForwardMsgTool extends StructuredTool {
  ...
  async _call(input: ReadToolInput, _manager: any, runnable: any): Promise<string> {
    try {
...
class SendForwardMsgTool extends StructuredTool {
  ...
  async _call(input: SendToolInput, _manager: any, runnable: any): Promise<string> {
    try {
      const checked = ensureOnebotSession(runnable, this.deps.config)
      if (isEnsureSessionFailure(checked)) return checked.error
      ...
      const items = parseToolMessages<SendMessageItem>(input.messages)
...
class SendFakeMsgTool extends StructuredTool {
  ...
  async _call(input: FakeToolInput, _manager: any, runnable: any): Promise<string> {
    try {
      const checked = ensureOnebotSession(runnable, this.deps.config)
      if (isEnsureSessionFailure(checked)) return checked.error
      ...
      const items = parseToolMessages<FakeMessageItem>(input.messages)
      if (!items.length) return 'messages is empty.'
...
class DescribeImageByUrlTool extends StructuredTool {
  deps: DescribeImageToolDeps
  name: string
  description: string
  schema: any

  constructor(deps: DescribeImageToolDeps) {
    super({})
    this.deps = deps
  ...
  async _call(input: DescribeToolInput): Promise<string> {
    try {
